...
export function DispatchManager() {
  ...
  function sendDetails(g: DispatchGroup) {
    ...
    lines.push(isInbound
      ? 'We have received your following document(s) at Hello Doctor:'
      : 'Your following document(s) have been dispatched from Hello Doctor:')
    lines.push(docList)
    lines.push('')
  ...
  function downloadReceipt(g: DispatchGroup) {
    ...
    const receiptNo = `HD-${(isInbound ? 'R' : 'D')}${(d.created_at ? new Date(d.created_at).getTime() : Date.now()).toString().slice(-8)}`
    ...
    <style>
      *{box-sizing:border-box;font-family:Arial,Helvetica,sans-serif;-webkit-print-color-adjust:exact;print-color-adjust:exact}
      ...
      <div class="wrap">
        <div class="head"><h1>Hello Doctor</h1><p>Document ${isInbound ? 'Received Acknowledgement' : 'Dispatch Receipt'}</p></div>
        <div class="body">
        ...
        <div class="foot">This is a system-generated receipt from Hello Doctor</div>
      </div>
