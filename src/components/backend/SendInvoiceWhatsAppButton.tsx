...
export function SendInvoiceWhatsAppButton({ student }: SendInvoiceWhatsAppButtonProps) {
    ...
    async function handleSend(e: React.MouseEvent) {
        ...
        try {
            ...
            // Branded, readable slug: hd-{name}-{course}-{session}-{unique}
            const idSuffix = student.id.replace(/-/g, '').slice(0, 6)
            const slug = `${slugify(['hd', student.full_name, course, session].filter(Boolean).join(' '))}-${idSuffix}`
            ...
            const invoiceUrl = `${window.location.origin}/crm/i/${slug}`
            ...
            const message =
                ...
                `Thank you for taking admission in ${courseLine} at Hello Doctor. ` +
