...
export default function AccountsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!user) { window.location.href = '/crm/student/login'; return }
      ...
      if (!s) { window.location.href = '/crm/student/login'; return }
      setStudent(s)
  ...
  async function handleShare() {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: 'Payment Summary — Hello Doctor', text })
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="px-5 py-4">
          ...
          <div className="space-y-2.5">
            {[
              { label: 'Account Name',   value: 'HELLO DOCTOR', mono: false },
              { label: 'Account Number', value: '00000000000', mono: true },
              { label: 'IFSC Code',      value: 'XXXX0000000', mono: true },
              { label: 'SWIFT Code',     value: 'XXXXXXXXXXX', mono: true },
              { label: 'Bank',           value: '(update pending)', mono: false },
