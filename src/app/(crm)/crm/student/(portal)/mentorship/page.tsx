...
export default function StudentMentorshipPage() {
  ...
  const isHd = mcase?.managed_by !== 'self'
          ...
          {!mcase ? (
            ...
            <>
              <div className="flex items-center gap-2 flex-wrap">
                <span className={`text-xs font-bold px-3 py-1 rounded-full ${isHd ? 'bg-blue-100 text-blue-700' : 'bg-gray-200 text-gray-700'}`}>{isHd ? 'Managed by Hello Doctor' : 'By Self'}</span>
              </div>
                  ...
                  return (
                    <div key={key} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                      <div className="flex items-center justify-between px-4 py-2.5 bg-gray-50 border-b border-gray-100">
                        ...
                        {isHd && stTotal > 0 && <span className="text-[11px] font-bold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-full">₹{stColl}/{stTotal}</span>}
                      </div>
