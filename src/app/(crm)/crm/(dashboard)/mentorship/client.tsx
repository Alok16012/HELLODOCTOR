...
export default function MentorshipClient() {
  ...
  const [caseMode, setCaseMode] = useState<'hd' | 'self'>('hd')
  ...
  function openManage(s: AssignedStudent) {
    ...
    setManageStudent(s)
    setCaseId(c?.id ?? null)
    setCaseMode((c?.managed_by as any) === 'self' ? 'self' : 'hd')
    ...
    try {
      ...
      const built = await Promise.all(STAGE_ORDER.map(async st => {
        const subs = await Promise.all(stages[st.key].map(async sub => {
          ...
          return { name: sub.name, amount: caseMode === 'hd' ? num(sub.amount) : 0, status: sub.status, payments: caseMode === 'hd' ? payments : [] }
        }))
      ...
      const payload: any = {
        ...
        total_amount: caseMode === 'hd' ? total : null,
        stages: built,
                      ...
                      <td className="px-3 py-3">
                        {sum ? (
                          <div className="flex items-center gap-1 flex-wrap min-w-[110px]">
                            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md ${sum.c.managed_by === 'self' ? 'bg-gray-200 text-gray-700' : 'bg-blue-100 text-blue-700'}`}>{sum.c.managed_by === 'self' ? 'Self' : 'Hello Doctor'}</span>
        ...
        <DialogContent className="w-[calc(100%-1.5rem)] sm:max-w-xl max-h-[92vh] overflow-y-auto rounded-2xl">
          ...
          <div className="space-y-4 mt-1">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => setCaseMode('hd')} className={`py-2.5 rounded-xl text-sm font-bold border-2 ${caseMode === 'hd' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-500 border-gray-200'}`}>Managed by Hello Doctor</button>
            ...
            {/* Auto total summary (Hello Doctor) */}
            {caseMode === 'hd' && (
