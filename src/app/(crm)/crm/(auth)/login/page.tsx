    ...
    try {
      ...
      if (user) {
        ...
        if (!profile) {
          ...
          toast.error('This account has no CRM access. Ask the admin to add you in Settings → Users.')
          return
        }
        if (profile?.role === 'associate') {
          window.location.replace('/crm/associate')
          return
        }
      }
  ...
  return (
    <Card className="w-full max-w-5xl shadow-2xl overflow-hidden border-none mx-4">
      <div className="flex flex-col md:flex-row min-h-[600px]">
        ...
        <div className="w-full md:w-5/12 bg-[#1e3a6e] p-12 text-white flex flex-col justify-between relative overflow-hidden hidden md:flex">
          <div className="absolute inset-0 bg-gradient-to-br from-[#1e3a6e] to-[#142a52] opacity-90 z-0"></div>

          <div className="relative z-10 flex flex-col h-full">
            <div className="flex items-center gap-4 mb-12">
              <Image src="/brand-logo.png" alt="Hello Doctor Logo" width={64} height={64} className="w-16 h-16 object-contain bg-white rounded-2xl p-1" priority />
              <h1 className="text-3xl font-bold tracking-tight">Hello Doctor</h1>
            </div>
          ...
          <div className="relative z-10 mt-12 bg-white/10 p-6 rounded-2xl backdrop-blur-md border border-white/20 shadow-lg">
            <p className="italic text-blue-50 text-sm">
              "Hello Doctor has transformed how we handle our student management. Highly recommended platform!"
            </p>
        ...
        <div className="w-full md:w-7/12 p-8 md:p-16 bg-white flex flex-col justify-center relative">
          ...
          <div className="flex md:hidden items-center gap-3 mb-8 justify-center">
            <Image src="/brand-logo.png" alt="Hello Doctor Logo" width={48} height={48} className="w-12 h-12 object-contain" />
            <h1 className="text-2xl font-bold tracking-tight text-gray-900">Hello Doctor</h1>
          </div>

          <div className="max-w-md w-full mx-auto space-y-8">
            <div className="text-center md:text-left">
              <h3 className="text-3xl font-bold text-gray-900 tracking-tight">{isAssociate ? 'Associate Login' : 'Staff Login'}</h3>
              <p className="text-gray-500 mt-2 text-sm">
                {isAssociate ? 'Sign in to your associate partner portal.' : 'Sign in to the Hello Doctor CRM.'}{' '}
                <a href={isAssociate ? '/crm/login' : '/crm/login?as=associate'} className="text-blue-600 hover:underline">
                  {isAssociate ? 'Office staff?' : 'Associate?'}
                </a>
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              <Button type="submit" className="w-full rounded-xl h-12 shadow-lg shadow-blue-200 bg-[#1e3a6e] hover:bg-[#142a52] text-white transition-all font-semibold" disabled={loading}>
                {loading ? (
