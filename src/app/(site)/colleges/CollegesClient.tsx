...
import { streams, feeRanges, type College } from "@/data/colleges";
...
export default function CollegesClient({ colleges }: { colleges: College[] }) {
  ...
  const topCities = useMemo(() => [...new Set(colleges.map((c) => c.city).filter(Boolean))].sort(), [colleges]);
  const topExams = useMemo(() => [...new Set(colleges.flatMap((c) => c.exams))].sort(), [colleges]);
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [colleges, searchQuery, selectedStream, selectedCity, selectedExam, selectedType, selectedFee, minRating, sortBy]);
      ...
      <div className="bg-gray-50 min-h-screen">
        ...
        <div className="bg-white border-b border-gray-100">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
            <h1 className="text-2xl font-bold text-gray-900">Medical Colleges & Universities — India &amp; Abroad</h1>
            <p className="text-gray-500 text-sm mt-1">
              Showing <span className="font-semibold text-blue-600">{filtered.length}</span> of {colleges.length} options &bull; Fees and rankings are indicative — talk to our counsellors for the latest details
            </p>
          </div>
        </div>
                  ...
                  <div className="space-y-1">
                    {streams.map((s) => (
                      <label key={s.name} className="flex items-center gap-2.5 py-1 cursor-pointer group">
                        ...
                        <span className="text-xs text-gray-400">{colleges.filter((c) => c.streams.includes(s.name)).length}</span>
                      </label>
                    ))}
                  </div>
                </FilterSection>
