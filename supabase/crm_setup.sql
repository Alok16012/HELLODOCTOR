-- ============================================================
-- HELLO DOCTOR — CRM (/crm) FULL DATABASE SETUP
-- Supabase Dashboard → SQL Editor → New query → ye poori file paste → Run
--
-- Ye website wale Supabase project pe hi chalana hai (jahan colleges /
-- blogs / scholarships tables hain). Website ka data safe rahega.
--
-- NOTE: website ki purani `leads` table (contact form enquiries) ka naam
-- badal ke `enquiries` kar diya jaata hai, kyunki CRM ko apni `leads`
-- table chahiye. Website ka code bhi `enquiries` use karta hai ab.
-- Naya enquiry aate hi wo CRM Leads me bhi automatically aa jayega
-- (niche STEP 3 dekho).
-- ============================================================


-- ────────────────────────────────────────────────────────────
-- STEP 1: website `leads` → `enquiries`
-- ────────────────────────────────────────────────────────────
DO $$
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE public.leads RENAME TO enquiries;
    ALTER SEQUENCE IF EXISTS public.leads_id_seq RENAME TO enquiries_id_seq;
  END IF;
END $$;

-- (fresh DB ke liye — agar website setup.sql abhi tak nahi chala)
...
create policy "admin manage" on enquiries for all to authenticated
...
-- ────────────────────────────────────────────────────────────
-- STEP 2: CRM tables (Admission CRM — saari migrations order me)
-- ────────────────────────────────────────────────────────────
...
-- inbound = document received at Hello Doctor from university/institute
-- outbound = document dispatched from Hello Doctor to student/associate
...
-- ────────────────────────────────────────────────────────────
-- MIGRATION: 077_enrollment_number_hd_format.sql
-- ────────────────────────────────────────────────────────────
-- Change enrollment number default from ENR- prefix to HD- prefix
ALTER TABLE students
  ALTER COLUMN enrollment_number
  SET DEFAULT 'HD-' || floor(random() * 900000 + 100000)::text;

-- Update existing ENR-XXXXXXX or ENR-XXXXXXX(letter) → HD-XXXXXXX (digits only)
UPDATE students
SET enrollment_number = 'HD-' || regexp_replace(enrollment_number, '^ENR-([0-9]+).*$', '\1')
...
-- managed_by: 'dcw' (Hello Doctor-managed, has payment) or 'self' (student self, no payment)
...
-- ────────────────────────────────────────────────────────────
-- Mentorship redesign: one Hello Doctor "case" per student with a 3-stage journey,
...
-- ────────────────────────────────────────────────────────────
-- MIGRATION: 094_managed_by_hd.sql
-- ────────────────────────────────────────────────────────────
-- Rebrand mentorship managed_by value: 'dcw' → 'hd'
ALTER TABLE student_mentorships DROP CONSTRAINT IF EXISTS student_mentorships_managed_by_check;
UPDATE student_mentorships SET managed_by = 'hd' WHERE managed_by = 'dcw';
ALTER TABLE student_mentorships ALTER COLUMN managed_by SET DEFAULT 'hd';
ALTER TABLE student_mentorships DROP CONSTRAINT IF EXISTS student_mentorships_managed_by_check;
ALTER TABLE student_mentorships ADD CONSTRAINT student_mentorships_managed_by_check CHECK (managed_by IN ('hd', 'self'));
...
-- ────────────────────────────────────────────────────────────
-- MIGRATION: 096_hello_doctor_seed.sql
-- ────────────────────────────────────────────────────────────
-- Hello Doctor master data: courses, departments (+ sub-sections), sessions.
-- Sab CRM me Settings se edit ho sakta hai.
...
FROM (VALUES
  ('MBBS India',            'MCC – All India Quota (15%)'),
  ('MBBS India',            'State Quota (85%)'),
  ('MBBS India',            'Deemed University'),
  ('MBBS India',            'Private Medical College (Management / NRI)'),
  ('MBBS Abroad',           'Russia'),
  ('MBBS Abroad',           'Georgia'),
  ...
  ('MBBS Abroad',           'Nepal'),
  ('AYUSH & Dental',        'AACCC – All India Quota'),
  ('AYUSH & Dental',        'State Counselling'),
  ('AYUSH & Dental',        'Private / Deemed'),
  ('Paramedical & Nursing', 'Government College'),
  ('Paramedical & Nursing', 'Private College'),
  ('NEET PG',               'MCC – All India Quota'),
  ('NEET PG',               'State Quota'),
...
-- ────────────────────────────────────────────────────────────
-- STEP 3: Website enquiry → CRM lead (automatic)
-- Contact form / NEET predictor se jo bhi enquiry aayegi, wo CRM ke
-- Leads me source = 'website' ke saath ban jayegi.
-- ────────────────────────────────────────────────────────────
create or replace function public.enquiry_to_crm_lead()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  ...
  where NEW.course is not null and name ilike '%' || NEW.course || '%'
  order by length(name) limit 1;

  insert into leads (full_name, phone, email, city, course_id, source, status, metadata)
  values (
    coalesce(nullif(trim(NEW.name), ''), 'Website Enquiry'),
    right(regexp_replace(coalesce(NEW.phone, ''), '[^0-9]', '', 'g'), 10),
    ...
    jsonb_build_object(
      ...
      'course', NEW.course,
      'message', NEW.message,
      'form', coalesce(NEW.source, 'website')
    )
  );
  return NEW;
exception when others then
  -- CRM me lead na bane tab bhi website ka form fail nahi hona chahiye
  raise warning 'enquiry_to_crm_lead failed: %', sqlerrm;
  return NEW;
end;
$$;

drop trigger if exists on_enquiry_to_crm_lead on enquiries;
create trigger on_enquiry_to_crm_lead
  after insert on enquiries
  for each row execute function public.enquiry_to_crm_lead();


-- ────────────────────────────────────────────────────────────
-- STEP 4: Pehla CRM Admin banao
-- 1) Supabase Dashboard → Authentication → Users → "Add user" →
--    email + password daalo, "Auto Confirm User" ON rakho.
--    (Agar website admin wala user already hai to wahi use kar sakte ho.)
-- 2) Niche wali line me apna email daal ke ye block chalao.
-- ────────────────────────────────────────────────────────────
insert into profiles (id, email, full_name, role, is_active)
select u.id, u.email, 'Hello Doctor Admin', 'admin', true
from auth.users u
where u.email = 'admin@hellodoctor.com'   -- ← CRM admin email
